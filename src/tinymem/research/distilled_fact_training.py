...
def _truth_from_episode(episode: Any, count: int) -> tuple[int | None, ...]:
    from tinymem.research.delta_fact_data import replay
...
def trajectory_diagnostics(
    ...
    from tinymem.research.delta_fact_data import Episode
    from tinymem.research.oracle_fact_state import _state_for_texts
...
def build_example(episode: Any, feature_cache: Mapping[str, torch.Tensor], writer: DeltaSlotWriter) -> DistilledExample:
    ...
    from tinymem.research.delta_fact_data import Episode, replay
    from tinymem.research.oracle_fact_state import _state_for_texts
