...
from tinymem.research import (
    ...
    distilled_fact_replication,
    distilled_fact_training,
    oracle_fact_fit,
)
from tinymem.research.delta_fact_data import ROOM_PAIRS, Episode, replay
from tinymem.research.delta_fact_encoding import (
    _cached_feature,
    _queries,
    build_feature_cache,
)
from tinymem.research.delta_fact_evaluation import StateRecord, collect_states
from tinymem.research.delta_fact_fit import load_state_records, save_state_records
...
from tinymem.research.oracle_fact_state import _state_for_texts, oracle_records
...
def score_cell(reader, study: Path, protocol: dict, dataset, index: int) -> dict[str, Any]:
    ...
    study = Path(study)
    distilled_fact_protocol.require_training_seal(study, protocol)
    evaluation_cells = distilled_fact_replication.evaluation_cells(protocol)
    ...
    bridge = oracle_fact_fit.load_trained(reader, parent_protocol, parent_cell,
    ...
    if oracle_fact_fit.execution_record(reader) != parent_report.get("runtime"):
        raise ValueError("reader execution runtime differs from the frozen parent endpoint")
    parameters_before = oracle_fact_fit.checkpoint_tensors(reader, bridge)
    writer = distilled_fact_fit.load_writer(study, protocol, distilled_fact_replication.training_index(cell))
    ...
    diagnostics = distilled_fact_training.trajectory_diagnostics(writer, episodes, feature_cache)
    ...
    parameters_after = oracle_fact_fit.checkpoint_tensors(reader, bridge)
    ...
    report = {"schema": "distilled_fact_cell_v1", "cell": cell, "rows": len(rows),
              ...
              "state_bytes": STATE_BYTES, "runtime": oracle_fact_fit.execution_record(reader),
              "writer_runtime": distilled_fact_fit._runtime(),
    ...
    identity = {**distilled_fact_protocol.evaluation_identity(study, protocol, index),
                "training_seal_sha256": file_hash(study / "training_sealed.json")}
    distilled_fact_protocol.seal_directory(directory, identity)
    return report
...
def _parent_provenance(study: Path, protocol: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for cell in distilled_fact_replication.evaluation_cells(protocol):
