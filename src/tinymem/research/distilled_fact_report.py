...
from tinymem.research.distilled_fact_scoring import (
    ENDPOINTS,
    READ_MODES,
    STATE_BYTES,
...
def aggregate_study(study: Path, protocol: dict) -> dict[str, Any]:
    ...
    distilled_fact_protocol.verify_study(study, study / "source")
    ...
    for index, cell in enumerate(evaluation_cells):
        ...
        completion = distilled_fact_protocol.verify_completion(directory, identity)
        ...
        writer_index = distilled_fact_replication.training_index(cell)
        writer = distilled_fact_fit.load_writer(study, protocol, writer_index)
        ...
        recomputed_diagnostics = distilled_fact_training.trajectory_diagnostics(
