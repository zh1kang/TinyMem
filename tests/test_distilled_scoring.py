...
from tinymem.studies.distilled import scoring
from tinymem.studies.delta.data import build_dataset, replay
...
def test_tiny_study_scores_and_aggregates_all_read_rows(writer_study, fixed_beta, normalize_hidden):
    ...
    if fixed_beta is not None:
        ...
        protocol = distilled_protocol.prepare_study(
            root, study, protocol["snapshot"],
            distilled_protocol.settings(device="cpu", smoke=True, fixed_beta=fixed_beta,
                                             normalize_hidden=normalize_hidden), parent,
        )
    dataset = distilled_protocol.read_dataset(study / "dataset.json")
    distilled_protocol.prepare_features(fresh(), study, protocol, dataset)
    distilled_fit.train_cell(study, protocol, dataset, 0)
    distilled_protocol.seal_training(study, protocol)

    cell_report = scoring.score_cell(fresh(), study, protocol, dataset, 0)
    aggregate = distilled_report.aggregate_study(study, protocol)
