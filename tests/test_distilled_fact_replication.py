...
from tinymem.research import (
    ...
    distilled_fact_scoring,
)
from tinymem.research import distilled_fact_protocol as protocol_module
from tinymem.research import distilled_fact_replication as replication
...
def test_normalized_pairs_start_with_identical_parameters_and_rng():
    ...
    for seed in a["seeds"]:
        control = distilled_fact_fit._writer(32, a, seed)
        rng = torch.get_rng_state().clone()
        normalized = distilled_fact_fit._writer(32, b, seed)
...
def test_replication_pipeline_trains_each_writer_once_and_checks_identities(writer_study):
    ...
    for cell in protocol["cells"]:
        distilled_fact_fit.train_cell(study, protocol, dataset, cell["index"])
    protocol_module.seal_training(study, protocol)
    for cell in replication.evaluation_cells(protocol):
        distilled_fact_scoring.score_cell(fresh_reader(), study, protocol, dataset, cell["index"])
    result = distilled_fact_report.aggregate_study(study, protocol)
    ...
    from tinymem.research.distilled_replication_analysis import summarize_rows
...
def test_seed_bootstrap_is_paired_and_reports_one_seed_sensitivity():
    from tinymem.research.distilled_replication_analysis import paired_seed_estimate
...
def test_crossed_readers_reuse_identical_writer_states(writer_study, monkeypatch):
    from tinymem.research import oracle_fact_fit, oracle_fact_protocol
    from tinymem.research.delta_fact_fit import load_state_records

    root, old_study, old_protocol, fresh = writer_study
    parent_settings = oracle_fact_protocol.settings
    ...
    monkeypatch.setattr(oracle_fact_protocol, "settings", three_reader_fixture)
    parent = old_study.with_name("three-readers")
    spec = oracle_fact_protocol.settings(device="cpu", smoke=True)
    parent_protocol = oracle_fact_protocol.prepare_study(root, parent, old_protocol["snapshot"], spec)
    parent_data = protocol_module.read_dataset(parent / "dataset.json")
    for cell in parent_protocol["cells"]:
        oracle_fact_fit.train_cell(fresh(), parent, parent_protocol, parent_data, cell["index"])
    oracle_fact_protocol.seal_training(parent, parent_protocol)
    ...
    protocol_module.seal_training(study, protocol)
    for cell in replication.evaluation_cells(protocol):
        distilled_fact_scoring.score_cell(fresh(), study, protocol, dataset, cell["index"])
    report = distilled_fact_report.aggregate_study(study, protocol)
    ...
    identity = {**protocol_module.evaluation_identity(study, protocol, 1),
                "training_seal_sha256": distilled_fact_report.file_hash(study / "training_sealed.json")}
...
def test_equal_answer_totals_remain_exactly_tied_across_reader_strata():
    from tinymem.research.distilled_replication_analysis import summarize_rows, paired_seed_estimate
