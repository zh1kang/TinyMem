...
from tinymem.reader import adapter
...
def test_history_features_are_detached_cpu_copies_from_a_frozen_reader(tiny_reader):
    hidden = adapter.frozen_history_features(tiny_reader, (3, 4, 5, 6))
    ...
    torch.testing.assert_close(hidden, adapter.frozen_history_features(tiny_reader, (3, 4, 5, 6)), rtol=0, atol=0)
    with pytest.raises(ValueError, match="valid native token IDs"):
        adapter.frozen_history_features(tiny_reader, ())
    with pytest.raises(ValueError, match="valid native token IDs"):
        adapter.frozen_history_features(tiny_reader, (3, tiny_reader.model.config.vocab_size))
    tiny_reader.model.config.max_position_embeddings = 3
    with pytest.raises(ValueError, match="no truncation"):
        adapter.frozen_history_features(tiny_reader, (3, 4, 5, 6))
    tiny_reader.model.get_input_embeddings().weight.requires_grad_(True)
    with pytest.raises(ValueError, match="frozen"):
        adapter.frozen_history_features(tiny_reader, (3, 4))
...
def test_read_adapter_owns_exactly_the_lora_parameters_in_each_arm(tiny_reader):
    with pytest.raises(ValueError, match="PEFT reader"):
        adapter.configure_read_adapter(tiny_reader, trainable=True)
    attach_reader_lora(tiny_reader, rank=2, checkpointing=False)
    before = deepcopy(tiny_reader.model.state_dict())
    trainable = adapter.configure_read_adapter(tiny_reader, trainable=True)
    ...
    frozen = adapter.configure_read_adapter(tiny_reader, trainable=False)
    assert frozen == ()
    ...
    with pytest.raises(ValueError, match="boolean"):
        adapter.configure_read_adapter(tiny_reader, trainable=1)
