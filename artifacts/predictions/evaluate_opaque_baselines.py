...
parser.add_argument("--reader-gate", type=Path, required=True)
...
if gate["protocol"] == "opaque_qa1_reader_qualification_v1":
    adapter, adapter_hashes = Path(gate["adapter"]), gate["adapter_sha256"]
elif gate["protocol"] == "opaque_qa1_reader_continuation_v1":
    adapter, adapter_hashes = Path(gate_result["final_adapter"]), gate_result["adapter_sha256"]
else:
...
reader = load_qwen_reader(Path("data/raw/pretrained/qwen3-1.7b"), device=torch.device("mps"), dtype=torch.bfloat16)
...
protocol = {
    ...
    "query_aware_write": False, "measurements": "CPU_policy_write_and_MPS_generation_wall_time_not_peak_memory",
    ...
    for variant in ("opaque", "short"):
        for world_row in worlds:
            ...
            for method in methods:
                memory = None if method == "fingerprint" else reader.model.get_input_embeddings()(torch.tensor(materialized[method], device="mps", dtype=torch.long))
                for case, example in zip(cases, examples, strict=True):
                    ...
                    if method == "fingerprint":
                        ...
                        generated = generate_prefix_answer(reader, torch.tensor(example.before_ids, device="mps"), memory,
                                                           torch.tensor(example.after_ids, device="mps"), max_new_tokens=8)
                    torch.mps.synchronize()
                    read_seconds = time.perf_counter() - tick
                    answer_ce = None if method == "fingerprint" else float(prefix_answer_loss(
                        reader, torch.tensor(example.before_ids, device="mps"), memory,
                        torch.tensor(example.after_ids, device="mps"), torch.tensor(example.answer_ids, device="mps")))
