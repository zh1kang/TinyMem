...
parser.add_argument("--memory-runs", type=Path, nargs=6, required=True)
...
for run in args.memory_runs:
    ...
    training_run = Path(protocol["training_run"])
...
provenance = {"study_protocol_sha256": study_hash,
...
print(json.dumps({"accuracy": table, "comparisons": comparisons}, indent=2), flush=True)
