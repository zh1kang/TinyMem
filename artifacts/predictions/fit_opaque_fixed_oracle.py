...
def load_inputs(training_run, training_fit, study_path):
    ...
    data_path = Path(training["arguments"]["data"])
...
def main():
    ...
    parser.add_argument("--training-fit", type=Path, required=True)
    ...
    protocol = {
        ...
        "numerics": {"reader_dtype": "bfloat16", "code_dtype": "float32", "device": "mps",
    ...
    reader = load_qwen_reader(snapshot_path, device=torch.device("mps"), dtype=torch.bfloat16)
    reader.model = PeftModel.from_pretrained(reader.model, Path(training["adapter"]), is_trainable=False,
    ...
    reader.model.train()
    tensor_examples = [[tuple(torch.tensor(ids, device="mps") for ids in
