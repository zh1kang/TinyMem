...
data = Path(training["arguments"]["data"])
...
gate_run = Path(training["arguments"]["reader_gate"])
...
writer = NativeRecurrentMemory(2048, memory_width=8, slots=2, segment_length=512,
                               writer_kind=training["writer_kind"], aggregation_width=training["aggregation_width"]).to("mps")
...
with torch.inference_mode():
    for variant in ("opaque", "counterfactual", "short"):
        ...
        for row in worlds:
            ...
            for chunk in chunks:
                state = writer.write(reader, state, torch.tensor(chunk, device="mps"))
            ...
            for index, (world, cases, examples, state) in enumerate(values):
                ...
                for condition, current in conditions.items():
                    for case, example in zip(cases, examples, strict=True):
                        torch.mps.synchronize()
                        tick = time.perf_counter()
                        generated = generate_prefix_answer(reader, torch.tensor(example.before_ids, device="mps"), current,
                                                           torch.tensor(example.after_ids, device="mps"), max_new_tokens=8)
                        torch.mps.synchronize()
                        read_seconds = time.perf_counter() - tick
                        answer_ce = float(prefix_answer_loss(reader, torch.tensor(example.before_ids, device="mps"), current,
