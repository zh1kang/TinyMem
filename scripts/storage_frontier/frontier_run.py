...
from tinymem.research.adapted_readout import (
...
from tinymem.research.storage_frontier_training import (
    answer_loss,
    encode_answer,
    optimizer_step,
    rollout,
...
def checked(study: Path) -> tuple[dict, dict]:
    ...
    for function, relative in ((frozen_history_features, 'tinymem/research/adapted_readout.py'),
...
def features(study: Path, protocol: dict, data: dict) -> dict[str, torch.Tensor]:
    ...
    if (seal['protocol_sha256'] not in sealed_protocol_hashes(study, protocol)
            ...
            or seal['encoder_sha256'] != sha(study / 'source/src/tinymem/research/adapted_readout.py')
...
def prepare_features(reader, study: Path, data: dict) -> None:
    ...
    write(directory / 'complete.json', {'dataset_sha256': sha(study / 'inputs/dataset.json'),
                                       ...
                                       'encoder_sha256': sha(study / 'source/src/tinymem/research/adapted_readout.py'),
                                       'runtime': runtime(),
...
def train(reader, study: Path, protocol: dict, data: dict, cell: int) -> None:
    from tinymem.research.storage_frontier_fit import fit
...
def load_checkpoint(reader, study: Path, protocol: dict, cell: int):
    ...
    declaration = protocol['cells'][cell]
    from tinymem.research.storage_frontier_fit import _base_parameters, _hash_parameters
